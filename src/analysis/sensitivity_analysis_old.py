...
class SobolAnalyzer:
    ...
    def calculate_indices(self, n_samples: int = 1024, 
        ...
        for i, param in enumerate(self.param_names):
            ...
            # Clamp to reasonable range [-1, 1] to handle numerical instability
            index_val = S_i / total_variance if total_variance > 0 else 0
            first_order[param] = max(-1.0, min(1.0, index_val))
        ...
        for i, param in enumerate(self.param_names):
            ...
            # Clamp to reasonable range [-1, 1] to handle numerical instability
            index_val = S_T / total_variance if total_variance > 0 else 0
            total_indices[param] = max(-1.0, min(1.0, index_val))
